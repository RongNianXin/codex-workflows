...
| 工具状态 | **可用；Windows；未签名** |
| 最后核验 | 2026-08-30 |
| 证据边界 | 多案例对照与单变量本地实验；上游永久修复版本仍待确认 |

> 安全第一步：先只读扫描。只有确认命中本文描述的扩展路径故障后，才退出 Codex 并执行修复。
...
正式修复前，工具要求 Codex 完全退出，原因是活动任务可能同时写库或把扩展路径重新写回。工具会在开始修复和真正写入前重复检查 Codex 进程，缩小用户误启动造成的并发窗口。
...
修复已经回读验证成功后，工具不会仅因为用户随后重新启动 Codex 而回滚已完成的修改。旧版曾在成功验证后再次检查进程，可能与用户手动归档产生竞态，出现“任务已成功归档，但工具仍报告失败”；当前版本已移除这个后置回滚触发点。GUI 还会在启动修复前先做一次友好检查，错误输出会清除 PowerShell 颜色控制码。
...
仓库随附的 EXE 没有发布者数字签名。仓库质量检查会从同目录 C# 源码重新编译一个临时副本，并核对编译结果内嵌的 PowerShell 脚本与当前源码一致；这能发现脚本版本错配，但不能替代代码签名或证明随附 EXE 的全部 GUI 字节都由当前源码构建。需要更高信任级别时，应在可信环境中自行审阅并编译源码。

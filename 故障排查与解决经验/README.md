...
- [CC Switch API 登录后 Codex 跨任务通信异常](<CC Switch API 登录后 Codex 跨任务通信异常/排查记录与建议.md>)
...
| 跨任务发送显示成功，但目标任务出现空回合、沿用旧输出或没有回执 | 分开核对投递、接口可见性、目标回复和反向 ACK；平台完成或空字符串不等于目标已处理 | [API 登录后跨任务通信异常](<CC Switch API 登录后 Codex 跨任务通信异常/排查记录与建议.md>) |
...
| 出现 `invalid paginated history lineage` 或 cutoff 超出父会话 | 父会话被截短后，子分段保留的字节偏移失效 | [切换账号后旧对话无法继续](<CC switch切换账号后，旧的对话无法继续/Codex 切换账号后旧对话无法继续.md>)；先停止迁移并保留现场 |
...
| 任务记录仍存在，但原窗口显示 `thread not found` | 先检查归档状态；必要时通过任务 ID 发送受限恢复消息，但它会创建新请求并写入状态 | [`thread not found` 恢复方案](<thread not found/thread-not-found-恢复方案.md>) |
| 需要审查历史任务迁移方案或回滚已有备份 | 当前真实安装已暂停；只可使用虚构测试和已有备份的回滚能力 | [历史任务批量迁移工具](<CC switch切换账号后，旧的对话无法继续/codex-bulk-session-migration/README.md>) |
...
- 密文迁移是最后手段：当前公开工具的真实安装入口因分页谱系风险已经暂停，不得绕过失败关闭。

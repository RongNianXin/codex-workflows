...
1. Codex 可以保持运行，直接双击 EXE。
2. 点击“一键扫描并修复全部异常”。
3. 工具先做在线只读扫描；没有异常就直接结束。
4. 只有发现异常时，工具才显示修复确认框。
5. 此时完全退出 Codex，包括系统托盘中的常驻进程，再点击“是”。
6. 工具会在离线状态重新扫描，通过安全检查后才修复；不会沿用可能过期的在线扫描结果。

目的：允许日常在线检查；只有真正需要写入修复时才要求退出 Codex。在线扫描的计数和异常列表来自同一个 SQLite 只读快照。

预期结果：

```text
RESULT_CODE=BULK_FAULTS_FOUND
只读扫描发现已知路径异常：正式修复前请完全退出 Codex。当前未修改数据库。
```

退出 Codex 并确认修复后，预期显示：
...
异常处理：如果确认修复后仍提示 Codex 正在运行，说明窗口或系统托盘进程没有完全退出；退出后重新点击按钮。若提示文件缺失、路径冲突或数据库完整性失败，停止操作并保留完整输出，不要创建空文件或手工改库。
...
正式修复前，工具要求 Codex 完全退出，原因是活动任务可能同时写库或把扩展路径重新写回。工具会在开始修复、真正写入前和成功返回前重复检查 Codex 进程，缩小用户误启动造成的并发窗口。

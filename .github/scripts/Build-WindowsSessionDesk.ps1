[CmdletBinding()]
param([ValidatePattern('^[0-9]+\.[0-9]+\.[0-9]+-dev\.[0-9]+$')][string]$Version='0.2.0-dev.3')
...
foreach($process in @(Get-CimInstance Win32_Process -Filter "Name = 'powershell.exe'")){
    ...
    try{
        ...
        throw ('无法安全退出旧工作台。请在旧页面点击“退出本机服务”，看到退出提示后重试。不要结束所有 PowerShell 进程。 / Cannot safely stop the old desk. Click Stop local service in its page, then retry. '+$_.Exception.Message)
    }
}
...
如提示文件占用，在旧网页点击“退出本机服务”。不要结束全部 PowerShell 进程。
...
If files are locked, click Stop local service in the old page. Never stop all PowerShell processes.

...
(async () => {
  ...
  try {
    ...
    await page.getByRole('link', { name: '场景 2G：保持功能不变，精简现有代码', exact: true }).click();
    const g = await page.getByRole('heading', { name: '场景 2G：保持功能不变，精简现有代码', exact: true }).boundingBox();
    assert(g.y >= 65 && g.y < 150, '2G anchor position');
    await page.getByRole('link', { name: /^2F-3：/ }).click();
    const f = await page.getByRole('heading', { name: /^2F-3：/ }).boundingBox();
    assert(f.y >= 65 && f.y < 150, '2F-3 anchor position');
    ...
    await page.getByRole('link', { name: '场景 2F：执行与独立审查协作', exact: true }).click();
    ...
    const permissionResult = await page.evaluate(async () => {
      ...
      const denied = document.getElementById('content').textContent === '' && entry.error === '需要重新授权';

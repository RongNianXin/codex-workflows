...
[打开整合工作台](session-desk-validation.html)：搜索与收藏任务、主动选择分段、检查完整性、查看统计摘要与详情。所有结果来自所选虚构文件，保存在本次页面内存中；刷新清除。支持同名任务区分，拒绝跨任务文件混用，不接入真实会话或自动目录发现。参见[使用与验证边界](SESSION_DESK_VALIDATION.md)。

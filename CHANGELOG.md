...
- 自然语言入口按目的、状态、对象和权限选择现有场景，补齐入口表中的遗漏；发布或推送意图直接加载交付与 PR 标准，无需 PR 时不强制创建，已有有效授权不重复索取。示例不作为触发密码，解释、反馈和暂停不自动扩展为业务执行。

- 操作手册将代码精简独立为场景 2G，保留旧 2B-1 的定位兼容；明确 2B 本地开发、2G 精简和 2F 协作的关系，沿用既有执行分工，由 AI 核实可恢复的参数，保持验证与授权边界。

- 代码精简先定义实际收益与逐项不退化护栏，统计包含抽取模块和新增依赖；区分行为保持、性能优化与算法改进，避免总评分掩盖关键退化。
- PR 复现补齐资源可获得性、目标构建/部署入口、本机来源与跨机内容身份的区别，以及合并前验证和合并后收口；基础命令关联本次改动契约，性能按同机对照与支持设备证据判断。
- 跨机器效果/速度差异补充运行身份、固定输入、分段计时与中间结果对照；PR 标准引用该流程，区分 CI 覆盖、人工效果和专业验收，不给普通 PR 增加无关硬件清单。
- 2B 增加保持功能不变的代码精简入口：先核对可删除性与等价基线，再按授权小批次处理；报告同口径收益、回归和未验证项。沿用现有角色与独立审查，不把迁移 TypeScript 或压低行数作为通用要求。

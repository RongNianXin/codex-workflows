...
- 第二代总指挥 `00`、`02`、`10` 与轻量交接启动配置统一升版为 `2026-09-01.4`；新增耐久 `COMMIT-LEDGER` 与 `KEY_NODE` 精简保护、并存实现决议矩阵和运行身份交付门禁。操作者只说“请帮我部署，我要人工核验”即可触发自动候选恢复，AI 在完成当前任务后以可选、渐进示例帮助操作者提升表达，不把长提示词变成前置门禁或授权。
